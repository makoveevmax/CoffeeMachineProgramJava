...
class Machine {
    ...
    private int checkRemains(int needWater, int needMilk, int needCoffee) {
        if (needWater > 0 && needWater > water) return 1;
        if (needMilk > 0 && needMilk > milk) return 2;
        if (needCoffee > 0 && needCoffee > coffee) return 3;
        if (cups < 1) return 4;
        else return 0;
    }

    private int checkCoffee(String answer) {
        switch (answer) {
            case "1":
                return checkRemains(250, 0, 16);
            case "2":
                return checkRemains(350, 75, 20);
            case "3":
                return checkRemains(200, 100, 12);
        }
        return 0;
    }
